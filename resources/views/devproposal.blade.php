                            ...
                            <div class="posts-list">
                                <div class="feed-shared-author-dt">
                                                            ...
                                                            <div class="author-dts">
                                                                ...
                                                                <p class="notification-text font-small-4 pt-1">
                                                                    <span class="time-dt">Joined on
                                                                        {{ $item->user->created_at->format('d:M:y g:i
                                                                        A') }}</span>
                                                                </p>
                                                                <p class="notification-text font-small-4 pt-1">
                                                                ...
                                                                <p class="notification-text font-small-4 pt-1">
                                                                    <span class="time-dt">{{ $item->user->badge->name
                                                                        }}</span>
                                                                </p>
                                                            </div>
                                                ...
                                                <span class="job-badge ttcolor">
                                                    @if (\Carbon\Carbon::parse(now())->diffInDays($item->days, false) <=
                                                        ...
                                                        false) < 60 && \Carbon\Carbon::parse(now())->
                                                            ...
                                                            {{ \Carbon\Carbon::parse(now())->diffInMinutes($item->days,
                                                            false) }}
                                                            Minutes left
                                                            @elseif(\Carbon\Carbon::parse(now())->diffInMinutes($item->days,
                                                            false) <= 0) @if (\Carbon\Carbon::parse(now())->
                                                                diffInSeconds($item->days, false) > 0)
                                                                {{
                                                                \Carbon\Carbon::parse(now())->diffInSeconds($item->days,
                                                                false) }}
                                                                Seconds left
                                                                @else
                                                                @if ($item->propsolution()->count() >= 1 &&
                                                                $item->propsolution->proposal_id == $item->id)
                                                                Closed
                                                                @else
                                                                Unsolved
                                                                @endif
                                                                ...
                                                                @else
                                                                {{
                                                                \Carbon\Carbon::parse(now())->diffInHours($item->days,
                                                                false) }}
                                                                Hours left
                                                                @endif
                                                                @else
                                                                {{ \Carbon\Carbon::parse(now())->diffInDays($item->days,
                                                                false) }}
                                                                days left
                                                                @endif
                                                </span>
                                    ...
                                    <div class="ellipsis-options post-ellipsis-options dropdown dropdown-account">
                                        ...
                                        <a href="" class="label-dker post_categories_top_right mr-20 ms-2"><span>{{
                            ...
                            <div class="post-meta">
                                <div class="job-actions">
                                    <div class="aplcnts_15">
                                        <i class="feather-users mr-2"></i><span>Applied</span><ins>{{
                                            $item->proposalbid()->count() }}</ins>
                                        <i class="feather-eye mr-2"></i><span>Views</span><ins>{{ $item->view_count
                                            }}</ins>
                                    </div>
...
<!--Add Proposal Model-->
<div class="modal fade" id="addproposal" tabindex="-1" data-bs-backdrop="static" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
                    ...
                    <form class="form p-3" method="POST" id="proposal" enctype="multipart/form-data"
                        ...
                        <div class="form-group pt-2">
                            ...
                            <input type="file" class="form-control" name="file" accept="image/*,.pdf" id="file"
                                value="{{ old('file') }}" placeholder="Upload image or
                                pdf">
                            <div class="text-danger mt-2 text-sm file"></div>
                        </div>
                        <input type="submit" @disabled($errors->isNotEmpty()) class="btn btn-primary bg-primary mt-3
                        w-100" name="submit"
                        value="Submit">
                    </form>
                </div>
